...
export default function Lightbox({
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "+" || e.key === "=") setScale((s) => clampScale(s * 1.15));
      if (e.key === "-") setScale((s) => clampScale(s / 1.15));
    };
  ...
  const onTouchEnd: React.TouchEventHandler<HTMLDivElement> = () => { pinch.current = null; };

  return (
    <div className="lb" onClick={onClose} onWheel={onWheel} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
