...
export default function HeroImagesUploader({ images, onChange, max = 10, disabled }: Props) {
  const inputRef = useRef<HTMLInputElement | null>(null);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const remaining = Math.max(0, max - (images?.length || 0));
  ...
  async function handleFiles(files: FileList | null) {
    ...
    try {
      ...
      for (const f of arr) {
        ...
        if (!res.ok) {
          if (res.status === 501) throw new Error("Загрузка отключена на Vercel (локальная запись недоступна).");
          throw new Error(j?.error || "upload failed");
        }
        uploaded.push(String(j.url));
      }
  ...
  async function removeOne(url: string) {
    if (!disabled && url.startsWith("/uploads/")) {
      try {
  ...
  return (
    <div>
      <div className="text-xs text-gray-500 mb-1">
        Hero-изображения (до {max}). PNG/JPG/WebP/GIF/HEIC. Макс. 5MB/файл.
      </div>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
        {(images || []).map((url) => (
          <div key={url} className="relative rounded-lg overflow-hidden border bg-white">
            ...
            <button
              type="button"
              className="absolute top-1 right-1 bg-white/90 hover:bg-white text-rose-600 border rounded-full px-2 py-1 text-xs shadow"
